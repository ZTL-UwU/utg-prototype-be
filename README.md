...
| `AWS_STORAGE_BUCKET_NAME` | unset                                             | When set, game media (word/sentence audio, word/mascot/reward images) is stored on S3 |
| `AWS_PRIVATE_STORAGE_BUCKET_NAME` | `AWS_STORAGE_BUCKET_NAME`                 | Bucket for private uploads (feedback images); required with `AWS_S3_CUSTOM_DOMAIN` |
...
| `AWS_S3_CUSTOM_DOMAIN`   | unset                                               | Public CDN domain for the game media bucket; its URLs are unsigned and cached for a year |
| `AWS_S3_SIGNATURE_VERSION` | `s3v4`                                            | S3 request signing (SigV4; required by R2 etc.) |
| `AWS_QUERYSTRING_AUTH`   | `true`                                              | Use signed URLs for media (ignored for game media when `AWS_S3_CUSTOM_DOMAIN` is set) |
